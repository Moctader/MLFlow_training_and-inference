...
if __name__ == "__main__":
    ...
    target_length = 100  # Example target length for demonstration
    ...
    for i, (method_name, original_data, consolidated_data) in enumerate(methods, start=1):
        ...
        plt.plot(np.linspace(0, len(original_data) - 1, num=target_length), consolidated_data, label=f'Consolidated Reference Data - {method_name}', marker='x')
        plt.xlabel('Index')
        plt.ylabel('Value')
        plt.title(f'Data Before and After Consolidation - {method_name}')
        plt.legend()
        plt.tight_layout()
    plt.show()