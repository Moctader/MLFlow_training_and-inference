...
class DataProcessor:
    ...
    def split_data(self, data, reference_ratio=0.02, current_ratio=0.01):
...
class ConceptDriftDetector:
    ...
    def run(self):
        ...
        for value in self.current_data_1:
            self.page_hinkley.update(value)
            if self.page_hinkley.drift_detected:
                print("Concept drift detected in current_data_1")
                mlflow.log_param("page_hinkley_drift_current_data_1", True)
                break
        else:
            mlflow.log_param("page_hinkley_drift_current_data_1", False)

        for value in self.current_data_2:
            self.page_hinkley.update(value)
            if self.page_hinkley.drift_detected:
                print("Concept drift detected in current_data_2")
                mlflow.log_param("page_hinkley_drift_current_data_2", True)
                break
        else:
            mlflow.log_param("page_hinkley_drift_current_data_2", False)
...
if __name__ == "__main__":
    ...
    reference_data, current_data_1, current_data_2 = processor.split_data(data['close'])
    reference_target, current_target_1, current_target_2 = processor.split_data(data['target'])
